...
async function getBalanceAndNonce(address) {
  const [bRes, nRes] = await Promise.all([
    fetch(`${API}/extended/v1/address/${address}/balances`),
    fetch(`${API}/extended/v1/address/${address}/nonces`),
  ]);
...
async function main() {
  ...
  // Distribute TOTAL_TXS round-robin across funded accounts
  const plan = [];
  ...
  // Group by account so we can assign sequential nonces
  ...
  // Flatten plan to a single ordered sequence with per-account nonce bookkeeping
  const nonceByAddr = new Map();
  for (const [addr, items] of byAddr.entries()) {
    nonceByAddr.set(addr, items[0].acct.nonce);
  }

  // Interleave so we don't pound one account in a row (better load distribution
  // and gives the API some breathing room between same-sender broadcasts).
  const seq = [];
  ...
  fs.writeFileSync('./sim-counter-100-results.json', JSON.stringify({
  ...
  console.log('Results saved to sim-counter-100-results.json');
}
